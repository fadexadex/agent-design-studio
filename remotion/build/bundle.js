...
const useTextSplitter = (text, unit) => {
...
const useAnimatableUnitCount = (units) => {
  return (0,react.useMemo)(() => {
    return units.filter((u) => !u.isSpace).length;
  }, [units]);
};

;// ./src/components/AnimatedText/utils/interpolation.ts


const mapEasing = (easing) => {
  if (!easing) return void 0;
  switch (easing) {
    case "linear":
      return esm.Easing.linear;
    case "easeIn":
      return esm.Easing.in(esm.Easing.quad);
    case "easeOut":
      return esm.Easing.out(esm.Easing.quad);
    case "easeInOut":
      return esm.Easing.inOut(esm.Easing.quad);
    case "cubicIn":
      return esm.Easing.in(esm.Easing.cubic);
    case "cubicOut":
      return esm.Easing.out(esm.Easing.cubic);
    case "cubicInOut":
      return esm.Easing.inOut(esm.Easing.cubic);
    case "backIn":
      return esm.Easing.in(esm.Easing.back(1.7));
    case "backOut":
      return esm.Easing.out(esm.Easing.back(1.7));
    case "backInOut":
      return esm.Easing.inOut(esm.Easing.back(1.7));
    default:
      return void 0;
  }
};
const safeInterpolate = (frame, inputRange, outputRange, easing) => {
  return (0,esm.interpolate)(frame, inputRange, outputRange, {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: mapEasing(easing)
  });
};
const interpolateWithDelay = (frame, delay, duration, from, to, easing) => {
  const startFrame = delay;
  const endFrame = delay + duration;
  return safeInterpolate(frame, [startFrame, endFrame], [from, to], easing);
};

;// ./src/components/AnimatedText/presets.ts
...
;// ./src/components/AnimatedText/hooks/useAnimationCalculator.ts





const normalizeConfig = (config, defaults) => {
  if (config === void 0 || config === false) return null;
  if (config === true) return defaults;
  return { ...defaults, ...config };
};
const isSpringConfig = (easing) => {
  return typeof easing === "object" && (easing == null ? void 0 : easing.type) === "spring";
};
const parseStagger = (stagger) => {
  if (typeof stagger === "number") {
    return { delay: stagger, reverse: false };
  }
  return { ...DEFAULTS.stagger, ...stagger };
};
const useAnimationCalculator = (props, totalUnits) => {
  ...
  return (0,react.useMemo)(() => {
    ...
    const blurConfig = normalizeConfig(
      blurProp ?? presetValues.blur,
      DEFAULTS.blur
    );
    const opacityConfig = normalizeConfig(
      opacityProp ?? presetValues.opacity,
      DEFAULTS.opacity
    );
    const scaleConfig = normalizeConfig(
      scaleProp ?? presetValues.scale,
      DEFAULTS.scale
    );
    const positionConfig = normalizeConfig(
    ...
    const getValuesForUnit = (unitIndex, isSpace) => {
      if (isSpace) {
        return {
          blur: 0,
          opacity: 1,
      ...
      const staggerDelay = stagger.reverse ? (totalUnits - 1 - unitIndex) * (stagger.delay || 0) : unitIndex * (stagger.delay || 0);
      let blur = 0;
      let opacity = 1;
      let scale = 1;
      let translateX = 0;
      ...
      if (positionConfig) {
        ...
        if (isSpringConfig(positionConfig.easing)) {
          ...
          translateX = (positionConfig.fromX ?? 0) + springValue * ((positionConfig.toX ?? 0) - (positionConfig.fromX ?? 0));
          translateY = (positionConfig.fromY ?? 0) + springValue * ((positionConfig.toY ?? 0) - (positionConfig.fromY ?? 0));
        } else {
      ...
      if (exit && exit.startFrame !== void 0 && frame >= exit.startFrame) {
...
;// ./src/components/AnimatedText/hooks/useStyleComposer.ts

const useStyleComposer = () => {
  const composeStyles = (values, scaleOrigin, positioning) => {
    ...
    const styles = {
      ...
      // Required for transforms on text
      opacity
    };
    if (blur > 0) {
      styles.filter = `blur(${blur}px)`;
    }
    if (positioning == null ? void 0 : positioning.isPositioned) {
      const allTransforms = [
  ...
  const getTransformString = (values) => {
    const transforms = [];
...
const getScaleOrigin = (scaleConfig) => {
...
;// ./src/components/AnimatedText/hooks/usePositioning.ts



const anchorToPercent = {
  center: { x: 50, y: 50 },
  top: { x: 50, y: 0 },
  bottom: { x: 50, y: 100 },
  left: { x: 0, y: 50 },
...
const parseOffset = (offset, dimension) => {
...
const parseCoordinate = (coord, dimension) => {
...
const getCenterTransform = (anchor) => {
  const offsets = {
...
const usePositioning_isSpringConfig = (easing) => {
  return typeof easing === "object" && (easing == null ? void 0 : easing.type) === "spring";
};
function usePositioning(props) {
  const { width, height, fps } = (0,esm.useVideoConfig)();
  const frame = (0,esm.useCurrentFrame)();
  if (!props.anchor && props.x === void 0 && props.y === void 0 && !props.anchorAnimation) {
    return {
      positionTransform: "",
      centerTransform: "",
      isPositioned: false
      // Element stays in normal flow (flexbox, etc.)
    };
  }
  let finalX;
  let finalY;
  ...
  if (explicitX !== void 0 || explicitY !== void 0) {
    ...
    const fromX = anchorToPercent[from].x / 100 * width + parseOffset(props.anchorAnimation.fromOffsetX, width);
    const fromY = anchorToPercent[from].y / 100 * height + parseOffset(props.anchorAnimation.fromOffsetY, height);
    const toX = anchorToPercent[to].x / 100 * width + parseOffset(props.anchorAnimation.toOffsetX, width);
    const toY = anchorToPercent[to].y / 100 * height + parseOffset(props.anchorAnimation.toOffsetY, height);
    if (usePositioning_isSpringConfig(easing)) {
      const springValue = (0,esm.spring)({
      ...
      finalX = (0,esm.interpolate)(frame, [delay, delay + duration], [fromX, toX], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
        easing: mapEasing(easing)
      });
      finalY = (0,esm.interpolate)(frame, [delay, delay + duration], [fromY, toY], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
        easing: mapEasing(easing)
      });
...
;// ./src/components/AnimatedText/utils/gradients.ts
...
const getGradientStyles = (gradient) => {
  ...
  const gradientValue = type === "radial" ? `radial-gradient(circle, ${colorStops})` : `linear-gradient(${angle}deg, ${colorStops})`;
  return {
...
const AnimatedText = (props) => {
  ...
  const presetValues = getPreset(preset);
  const animationUnit = animationUnitProp ?? presetValues.animationUnit ?? "full";
  const units = useTextSplitter(text, animationUnit);
  const totalAnimatableUnits = useAnimatableUnitCount(units);
  const { getValuesForUnit, typewriterVisibleChars, showCursor } = useAnimationCalculator(props, totalAnimatableUnits);
  ...
  const isSlidePreset = preset == null ? void 0 : preset.toLowerCase().includes("slide");
  ...
  const positioningStyles = positioning.isPositioned ? {
    position: "absolute",
    left: 0,
    top: 0,
    ...maxWidth && {
  ...
  const isTypewriter = preset === "typewriter" || typewriter !== void 0 && typewriter !== false;
  if (isTypewriter && animationUnit === "character") {
    const displayedText = text.slice(0, typewriterVisibleChars);
    const typewriterConfig = typeof typewriter === "object" ? typewriter : { cursor: true };
    ...
    return /* @__PURE__ */ (0,jsx_runtime.jsxs)(
      "span",
      {
        ...
        children: [
          displayedText,
          typewriterConfig.cursor !== false && /* @__PURE__ */ (0,jsx_runtime.jsx)(
            "span",
            {
              style: {
  ...
  if (animationUnit === "full") {
    ...
    if (shouldMask) {
      ...
      const { transform: animationTransform, ...otherAnimationStyles } = animationStylesWithoutPositioning;
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
    "span",
    {
      ...
      children: units.map((unit, index) => {
        ...
        if (unit.isSpace) {
          return /* @__PURE__ */ (0,jsx_runtime.jsx)("span", { style: { whiteSpace: "pre" }, children: unit.text }, `space-${index}`);
        }
        if (shouldMask) {
...
;// ./src/components/AnimatedText/LayoutGrid.tsx
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
    "div",
    {
      style: {
        ...
        left: isPositioned ? 0 : 0,
        // Reset default flow if absolute
        top: isPositioned ? 0 : 0,
        transform: transform || void 0,
        // Apply composed transform
        // Size Constraints
        width: isPositioned ? void 0 : "100%",
        // Full width in flow, auto in absolute
        height: isPositioned ? void 0 : "100%",
        maxWidth: typeof maxWidth === "number" ? `${maxWidth}px` : maxWidth,
        zIndex,
...
;// ./src/components/AnimatedText/TextSequence.tsx





const TextSequenceItem = ({
  duration,
  delay,
  exitPreset,
  ...animatedTextProps
}) => {
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(AnimatedText, { ...animatedTextProps });
};
const TextSequence = ({ mode = "chain", overlap = 0, children }) => {
  const { fps } = (0,esm.useVideoConfig)();
  let runningFrame = 0;
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(jsx_runtime.Fragment, { children: react.Children.map(children, (child, index) => {
...
;// ./src/components/AnimatedText/index.ts
...
;// ./src/components/Global/Background/presets.ts
...
const getBackgroundPreset = (name) => backgroundPresets[name];
const getRandomBackgroundPreset = () => {
  const presets = Object.values(backgroundPresets);
  return presets[Math.floor(Math.random() * presets.length)];
};
const listBackgroundPresets = () => Object.keys(backgroundPresets);
const getTypeVariantLayers = (type, variant = "light", meshColors) => {
  const BG_COLORS = {
    light: "#FFFFFF",
    dark: "#0F0F11",
    brand: "#4F46E5"
  ...
  const resolvedMeshColors = {
    primary: (meshColors == null ? void 0 : meshColors.primary) ?? DEFAULT_MESH_COLORS[variant].primary,
    secondary: (meshColors == null ? void 0 : meshColors.secondary) ?? DEFAULT_MESH_COLORS[variant].secondary
  };
  switch (type) {
    case "gradient-mesh":
      return [
...
;// ./src/components/Global/Background/utils/layerDefaults.ts
...
const RADIAL_DEFAULTS = {
  centerX: 50,
  centerY: 50,
  radius: void 0,
  // uses CSS default (farthest-corner)
  shape: "ellipse",
  opacity: 1
};
const MESH_DEFAULTS = {
...
;// ./src/components/Global/Background/hooks/useAnimationEngine.ts
...
function useAnimationEngine(options = {}) {
  ...
  const oscillate = (min, max, period = 300) => {
    if (!animated) return min;
    const p = Math.max(period, 1);
    return (0,esm.interpolate)(frame % p, [0, p / 2, p], [min, max, min]);
  };
  const rotate = (startAngle, period = 600) => {
    if (!animated) return startAngle;
    return (0,esm.interpolate)(frame, [0, period], [startAngle, startAngle + 360], {
      extrapolateRight: "wrap"
    });
  };
  const pulse = (base, amplitude, period = 200) => {
  ...
  const drift = (center, range, period = 400) => {
...
;// ./src/components/Global/Background/layers/SolidLayer.tsx
...
;// ./src/components/Global/Background/layers/LinearGradientLayer.tsx





const LinearGradientLayer = ({
  colors,
...
;// ./src/components/Global/Background/layers/RadialGradientLayer.tsx





const RadialGradientLayer = ({
  colors,
...
;// ./src/components/Global/Background/layers/MeshGradientLayer.tsx





const MeshGradientLayer = ({
  colors,
  points,
  ...
  const meshPoints = points ?? colors.map((_, i) => defaultPoints[i % defaultPoints.length]);
  const animatedPoints = meshPoints.map((pt, i) => ({
    x: drift(pt.x, 10 + i * 3, 400 + i * 80),
    y: drift(pt.y, 8 + i * 2, 350 + i * 70)
  }));
  const blobs = animatedPoints.map(
    (pos, i) => `radial-gradient(circle at ${pos.x}% ${pos.y}%, ${colors[i % colors.length]} 0%, transparent ${spread}%)`
  ).join(", ");
...
;// ./src/components/Global/Background/layers/NoiseLayer.tsx
...
;// ./src/components/Global/Background/layers/BlurLayer.tsx
...
;// ./src/components/Global/Background/layers/VignetteLayer.tsx
...
;// ./src/components/Global/Background/layers/GlowLayer.tsx
...
;// ./src/components/Global/Background/layers/GridLayer.tsx
...
const GridLayer = ({
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
    esm.AbsoluteFill,
    {
      ...
      children: /* @__PURE__ */ (0,jsx_runtime.jsxs)("svg", { width: "100%", height: "100%", xmlns: "http://www.w3.org/2000/svg", children: [
        ...
        /* @__PURE__ */ (0,jsx_runtime.jsx)("rect", { width: "100%", height: "100%", fill: "url(#background-grid)" })
      ] })
    }
  );
};

;// ./src/components/Global/Background/Background.tsx
...
const Background = ({
  ...
  if (preset && backgroundPresets[preset]) {
    ...
    animated = propAnimated !== void 0 ? propAnimated : presetConfig.animated ?? false;
    animationSpeed = propAnimationSpeed * (presetConfig.animationSpeed ?? 1);
  } else if (type) {
    layers = getTypeVariantLayers(type, variant, meshColors);
    animated = propAnimated !== void 0 ? propAnimated : type !== "grid-lines" && type !== "solid";
  ...
  const resolveAnimation = (layer) => ({
    animated: layer.animated !== void 0 ? layer.animated : animated,
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(
    esm.AbsoluteFill,
    {
      ...
      children: layers.map(renderLayer)
    }
  );
};

;// ./src/components/Global/Background/TestComposition.tsx
...
const Label = ({
  ...
  "div",
  {
    ...
    children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)("div", { style: { fontSize: 32, fontWeight: 700 }, children: text }),
      subtext && /* @__PURE__ */ (0,jsx_runtime.jsx)("div", { style: { fontSize: 18, opacity: 0.8, marginTop: 4 }, children: subtext })
    ]
  }
);
const PresetShowcase = () => {
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(Background, { preset: currentPreset }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)("div", { style: { opacity }, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(
      Label,
      {
        text: currentPreset,
...
const TypeVariantTest = () => {
  const frame = (0,esm.useCurrentFrame)();
  const types = ["gradient-mesh", "grid-lines", "blobs", "solid"];
  const variants = ["light", "dark", "brand"];
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(Background, { type, variant, animated: true }),
...
const CustomLayersTest = () => {
  const frame = (0,esm.useCurrentFrame)();
  const pulse = (0,esm.interpolate)(Math.sin(frame * 0.05), [-1, 1], [0.3, 0.6]);
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(
      Background,
      {
    ...
    /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Custom Layers", subtext: "Programmatic layer composition" })
  ] });
};
const LayerTypesGallery = () => {
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { display: "flex", flexWrap: "wrap" }, children: [
    /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { width: "33.33%", height: "50%", position: "relative" }, children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Background, { layers: [{ type: "solid", color: "#4F46E5" }] }),
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Solid" })
    ] }),
    /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { width: "33.33%", height: "50%", position: "relative" }, children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        Background,
      ...
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Linear" })
    ] }),
    /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { width: "33.33%", height: "50%", position: "relative" }, children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        Background,
      ...
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Radial" })
    ] }),
    /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { width: "33.33%", height: "50%", position: "relative" }, children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        Background,
      ...
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Mesh" })
    ] }),
    /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { width: "33.33%", height: "50%", position: "relative" }, children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        Background,
      ...
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Grid" })
    ] }),
    /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { width: "33.33%", height: "50%", position: "relative" }, children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)(
        Background,
      ...
      /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: "Glow+Noise+Vignette" })
    ] })
  ] });
};
const AnimationSpeedTest = () => {
  const speeds = [0.2, 0.5, 1, 2];
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.AbsoluteFill, { style: { display: "flex", flexWrap: "wrap" }, children: speeds.map((speed, i) => /* @__PURE__ */ (0,jsx_runtime.jsxs)(
    "div",
    {
      style: { width: "50%", height: "50%", position: "relative" },
      children: [
        /* @__PURE__ */ (0,jsx_runtime.jsx)(Background, { preset: "neonDream", animated: true, animationSpeed: speed }),
        /* @__PURE__ */ (0,jsx_runtime.jsx)(Label, { text: `Speed: ${speed}x` })
      ]
    },
    speed
  )) });
...
const BackgroundTest = () => {
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: 0, durationInFrames: 300, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(PresetShowcase, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: 300, durationInFrames: 180, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(TypeVariantTest, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: 480, durationInFrames: 120, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(CustomLayersTest, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: 600, durationInFrames: 150, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(LayerTypesGallery, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Sequence, { from: 750, durationInFrames: 150, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(AnimationSpeedTest, {}) })
  ] });
};
/* harmony default export */ const TestComposition = ((/* unused pure expression or super */ null && (BackgroundTest)));

;// ./src/components/Global/Background/index.ts
...
;// ./src/components/Global/BackgroundRig.tsx
...
const BackgroundRig = ({
  ...
  return /* @__PURE__ */ jsxs(
    AbsoluteFill,
    {
      ...
      children: [
        type === "grid-lines" && /* @__PURE__ */ jsxs("svg", { width: "100%", height: "100%", xmlns: "http://www.w3.org/2000/svg", children: [
        ...
        type === "gradient-mesh" && /* @__PURE__ */ jsxs("div", { style: { position: "relative", width: "100%", height: "100%" }, children: [
          /* @__PURE__ */ jsx(
            "div",
            {
              style: {
                ...
                transform: animate ? `translate(${Math.sin(animFrame / 60) * 50}px, ${Math.cos(animFrame / 70) * 30}px)` : "none",
                filter: "blur(60px)"
              }
            }
          ),
          /* @__PURE__ */ jsx(
            "div",
            {
              style: {
                ...
                transform: animate ? `translate(${Math.cos(animFrame / 80) * 40}px, ${Math.sin(animFrame / 60) * 40}px)` : "none",
                filter: "blur(60px)"
              }
            }
          )
        ] }),
        type === "blobs" && /* @__PURE__ */ jsxs("div", { style: { position: "relative", width: "100%", height: "100%" }, children: [
          /* @__PURE__ */ jsx(
            "div",
            {
              style: {
                ...
                transform: animate ? `scale(${1 + Math.sin(animFrame / 40) * 0.1})` : "none",
                filter: "blur(80px)"
              }
            }
          ),
          /* @__PURE__ */ jsx(
            "div",
            {
              style: {
                ...
                transform: animate ? `scale(${1 + Math.cos(animFrame / 50) * 0.1})` : "none",
                filter: "blur(80px)"
              }
            }
          )
...
;// ./src/components/Layout/MotionContainer.tsx
...
const MotionContainer = ({
  ...
  const hasExit = exit && exitStartFrame !== void 0;
  const exitProgress = hasExit ? (0,esm.spring)({
  ...
  const getEntranceStyles = () => {
    const styles = {};
    if (initial === "hidden" || initial === "offscreen-bottom" || initial === "offscreen-top" || initial === "offscreen-left" || initial === "offscreen-right" || initial === "scale-zero" || initial === "blur") {
      styles.opacity = (0,esm.interpolate)(progress, [0, 1], [0, 1], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp"
      });
    }
    ...
    if (initial === "offscreen-bottom") {
      const translateY = (0,esm.interpolate)(progress, [0, 1], [distance, 0], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp"
      });
      transforms.push(`translateY(${translateY}px)`);
    ...
    if (initial === "offscreen-left") {
      const translateX = (0,esm.interpolate)(progress, [0, 1], [-distance, 0], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp"
      });
      transforms.push(`translateX(${translateX}px)`);
    ...
    if (initial === "blur") {
      const blurAmount = (0,esm.interpolate)(progress, [0, 1], [10, 0], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp"
      });
  ...
  const combinedStyle = { ...style, ...animatedStyles };
  if (fill) {
    return /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.AbsoluteFill, { className, style: combinedStyle, children });
  }
  return /* @__PURE__ */ (0,jsx_runtime.jsx)("div", { className, style: combinedStyle, children });
};
...
;// ./src/generated/scenes/Scene1.tsx
...
;// ./src/generated/scenes/Scene3.tsx
...
;// ./src/generated/scenes/Scene4.tsx
...
;// ./src/generated/scenes/Scene5.tsx
...
;// ./src/generated/scenes/Scene6.tsx
...
;// ./src/generated/scenes/MainComposition.tsx
...
const MainComposition = () => {
  return /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.AbsoluteFill, { style: { backgroundColor: "#0011ff" }, children: /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.Series, { children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Series.Sequence, { durationInFrames: 226, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(Scene1, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Series.Sequence, { durationInFrames: 225, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(Scene2, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Series.Sequence, { durationInFrames: 225, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(Scene3, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Series.Sequence, { durationInFrames: 225, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(Scene4, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Series.Sequence, { durationInFrames: 225, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(Scene5, {}) }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Series.Sequence, { durationInFrames: 225, children: /* @__PURE__ */ (0,jsx_runtime.jsx)(Scene6, {}) })
  ] }) });
};
/* harmony default export */ const scenes_MainComposition = ((/* unused pure expression or super */ null && (MainComposition)));

;// ./src/components/Layout/TestComposition.tsx
...
const TestComposition_Card = ({ title, color }) => /* @__PURE__ */ (0,jsx_runtime.jsxs)(
  "div",
  {
    ...
    children: [
      /* @__PURE__ */ (0,jsx_runtime.jsx)("h2", { style: { margin: 0, fontSize: 32, fontWeight: 700 }, children: title }),
      /* @__PURE__ */ (0,jsx_runtime.jsx)("p", { style: { opacity: 0.8, marginTop: 8 }, children: "Feature Description" })
    ]
  }
);
const LayoutTest = () => {
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { backgroundColor: "white" }, children: [
...
const FallbackComposition = () => /* @__PURE__ */ (0,jsx_runtime.jsx)("div", { style: {
  width: "100%",
  height: "100%",
  display: "flex",
  alignItems: "center",
...
}, children: /* @__PURE__ */ (0,jsx_runtime.jsx)("p", { children: "No generated composition yet. Run the workflow to generate scenes." }) });
const RemotionRoot = () => {
...
var renderContent = (Root) => {
  ...
  if (bundleMode.type === "index") {
    ...
    Promise.all(/* import() */[__webpack_require__.e(634), __webpack_require__.e(845)]).then(__webpack_require__.bind(__webpack_require__, 2634)).then(({ StudioInternals }) => {
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "remotion-renderer:";
